...
export async function runMavenAnalysis(
  ...
  try {
    ...
    console.log(
      `[Maven Analyzer] Successfully parsed analysis data with ${Object.keys(result).length} top-level keys`,
    );
    return result;
  } catch (error) {
    ...
    console.error(
      `[Maven Analyzer] JSON content preview: ${jsonContent.substring(0, 200)}...`,
    );
    throw error;
  }
}
